...
public final class StringUtil {
  ...
  public static int[] fromString(final String s, final int radix, final int maxWidth) {    
    ...
    // Fix: Ensure firstlen doesn't exceed remaining string length
    firstlen = Math.min(firstlen, len - pos);
    ...
    try {
      ...
      throw new NumberFormatException("Illegal digit in group: " + group);
    }

    final int superradix = RADIX_INT[radix - 2];
    int groupv = 0;
    
    // Fix: Add proper bounds checking and limit iterations
    int maxIterations = (len - pos + perint - 1) / perint; // Calculate max iterations
    int iterations = 0;
    
    while(pos < len && iterations < maxIterations) {
      int groupLen = Math.min(perint, len - pos);
      if(groupLen <= 0) break;
      
      group = s.substring(pos, pos + groupLen);
      pos += groupLen;
      
      try {
        groupv = Integer.parseInt(group, radix);
        if(groupv < 0)
          throw new NumberFormatException("Illegal digit");
      } catch (NumberFormatException e) {
        throw new NumberFormatException("Illegal digit in group: " + group);
    ...
    return NumericArrays.stripLeadingZeroes(ints);
  }

  private static void muladd(final int[] out, final int mul, final int add) {
    final long lmul = mul & Long.MAX_VALUE, ladd = add & Long.MAX_VALUE;
    final int len   = out.length;

    // Check for potential overflow before starting
    if (mul > Integer.MAX_VALUE || add > Integer.MAX_VALUE) {
      throw new ArithmeticException("Overflow in muladd operation");
    }

    long carry = 0;
    for(int outi = len - 1; 0 <= outi; outi--) {
      final long outVal = out[outi] & Long.MAX_VALUE;
      final long prod = lmul * outVal + carry;
      
      // Check for overflow in multiplication
      if (lmul != 0 && prod / lmul != outVal) {
    ...
    long sum = (out[len - 1] & Long.MAX_VALUE) + ladd;
    
    // Check for overflow in addition
    if (sum < (out[len - 1] & Long.MAX_VALUE) && ladd > 0) {
    ...
    for(int outi = len - 2; carry != 0L && 0 <= outi; outi--) {
      final long currentVal = out[outi] & Long.MAX_VALUE;
      sum = currentVal + carry;
      
      // Check for overflow in carry addition
      if (sum < currentVal && carry > 0) {
        throw new ArithmeticException("Overflow in carry addition");
      }
  ...
  public static String toString(final int[] ints, final int radix) { 
    ...
    // Fix: Add iteration limit to prevent infinite recursion
    int maxIterations = ints.length * 2; // Reasonable limit
    int iterations = 0;

    do {
      tmp = NumericArrays.divmod(q, divisor);
      q   = tmp[0];
      r   = tmp[1];
      if(r.length == 0)
        groups[group++] = "0";
        ...
        final long rl = r.length == 1 ?
          (r[0] & Long.MAX_VALUE) : ((r[0] & Long.MAX_VALUE) << 32) | (r[1] & Long.MAX_VALUE);
      ...
      if(iterations >= maxIterations) {
        throw new ArithmeticException("Too many iterations in toString conversion");
      }
    } while(0 < q.length);
