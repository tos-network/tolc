  ...
  implements Comparable<String>, CharSequence, Serializable
{
  ...
  private String(Object data, int offset, int length, boolean copy) {
    ...
    if(!copy && data instanceof byte[] && Utf8.test(data)) copy = true;

    if (copy) {
      char[] c;
      if (data instanceof char[]) {
        c = new char[length];
        System.arraycopy((char[])data, offset, c, 0, length);
      } else {
        // If data is byte[], decode it using Utf8
        byte[] bytes = (byte[])data;
        Object decoded = Utf8.decode(bytes, offset, length);
        if(decoded == null) {
        ...
        if (decoded instanceof char[]) {
          ...
          // If it's byte[], convert to char[]
          byte[] decodedBytes = (byte[])decoded;
          c = new char[decodedBytes.length];
          for (int i = 0; i < decodedBytes.length; i++) {
            c[i] = (char)(decodedBytes[i] & 0xFF);
          }
        }
      }
      ...
      if (data instanceof char[]) {
        ...
        // If data is byte[], we need to decode it first
        byte[] bytes = (byte[])data;
        Object decoded = Utf8.decode(bytes, offset, length);
        if(decoded == null) {
          throw new RuntimeException
        ...
        if (decoded instanceof char[]) {
          this.data = (char[])decoded;
          this.offset = 0;
          this.length = ((char[])decoded).length;
        } else {
          // If it's byte[], convert to char[]
          byte[] decodedBytes = (byte[])decoded;
          char[] chars = new char[decodedBytes.length];
          for (int i = 0; i < decodedBytes.length; i++) {
            chars[i] = (char)(decodedBytes[i] & 0xFF);
          }
          this.data = chars;
          this.offset = 0;
                       ...
                       byte[] dst, int dstOffset)
  {
    ...
    for (int i = 0; i < srcLength; ++i) {
      dst[i + dstOffset] = (byte) data[i + offset + srcOffset];
    }
  }
    ...
    throws java.io.UnsupportedEncodingException
  {
    ...
    if (DEFAULT_ENCODING.equals(fmt)) {
      return Utf8.encode(data, offset, length);
    } else {
                       ...
                       char[] dst, int dstOffset)
  {
    ...
    System.arraycopy(data, offset + srcOffset, dst, dstOffset, srcLength);
  }
