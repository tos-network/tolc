...
import static java.base.Stream.read1;
import static java.base.Stream.read2;
...
public class Classes {
    ...
    throws ClassNotFoundException
  {
    Class c;
    if (loader == null) {
      // When loader is null, we can't load the class
      // This is a simplified implementation for the VM
      throw new ClassNotFoundException("Class not found: " + name + " (no ClassLoader available)");
    } else {
      c = loader.loadClass(name.replace('/', '.'));
    }
  ...
  public static Class forCanonicalName(ClassLoader loader, String name) {
    try {
      if (name.startsWith("[")) {
        ...
        if (name.length() == 1) {
          // For primitive types, create a Class object directly
          // instead of calling SystemClassLoader.getClass which may not work properly
          VMClass vmClass = primitiveClass(name.charAt(0));
          if (vmClass != null) {
            try {
              // Create a Class object for the primitive type
              return new Class(vmClass);
            } catch (Exception e) {
              // If creating Class object fails, throw a more specific exception
              throw new ClassNotFoundException("Failed to create Class object for primitive: " + name, e);
            }
          } else {
            throw new ClassNotFoundException("Primitive class not found: " + name);
          }
        } else {
          if (loader == null) {
            throw new ClassNotFoundException("Class not found: " + name + " (no ClassLoader available)");
